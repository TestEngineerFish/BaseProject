    ...
    class func authorizeRemoteNotification(_ completion:@escaping (Bool) -> Void) {
        if #available(iOS 10.0, *) {
            ...
            if UIApplication.shared.isRegisteredForRemoteNotifications {
                let enabledRemoteNotificationTypes = UIApplication.shared.enabledRemoteNotificationTypes()
                if enabledRemoteNotificationTypes.contains(.alert) ||
                    enabledRemoteNotificationTypes.contains(.badge) ||
                    enabledRemoteNotificationTypes.contains(.sound) ||
                    enabledRemoteNotificationTypes.contains(.newsstandContentAvailability) {
                    completion(true)
                }else {
                    completion(false)
                }
...
extension YYAuthorizationManager: CLLocationManagerDelegate {
...
extension YYAuthorizationManager {
    ...
    class func showAlert(title: String, message: String) {
        YYAlertManager.showTwoHorizontalButtonAlertView(title: title, description: message, leftButtonName: "取消", leftButtonClosure: {
