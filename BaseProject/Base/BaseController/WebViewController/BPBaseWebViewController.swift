...
//    }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if funciontList.contains(funcName) {
            BPAlertManager.share.oneButton(title: "JS调用了没有参数函数", description: "函数名:" + funcName + "参数是 : \(parameters)", buttonName: "知道了", closure: nil).show()
        }
    }
    ...
    func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping () -> Void) {
        BPAlertManager.share.oneButton(title: "捕获到JS的弹框", description: message, buttonName: "知道了", closure: completionHandler).show()
    }
