...
class BPDatabaseManager {
    ...
    private func normalRunner() throws -> FMDatabase {
        let filePath = self.dbFilePath(fileName: BPDatabaseType.normal.rawValue)
        print(filePath)
        return try createRunner(type: .normal, filePath: filePath, sqls: BPSQLManager.createNormalTables)
    }
