...
class BPSocketManager: NSObject, GCDAsyncSocketDelegate {
    ...
    func connection(toHost host: String, onPort port: UInt16) {
        ...
        do {
            ...
            self.delegate?.updateEvent("连接成功")
        } catch {
    ...
    func sendData(_ data: Data?) {
        ...
        self.delegate?.updateEvent("消息发送成功")
    }
    ...
    func addTimer() {
        timer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) { (timer) in
            let data = String(format: "%@", "heartbtes test").data(using: .utf8)
    ...
    func socket(_ sock: GCDAsyncSocket, didRead data: Data, withTag tag: Int) {
        ...
        self.delegate?.updateEvent("接收到数据: " + dataStr)
        self.clientSocket.readData(withTimeout: -1, tag: 0)
    }
    ...
    func socketDidDisconnect(_ sock: GCDAsyncSocket, withError err: Error?) {
        ...
        self.timer = nil
        self.delegate?.updateEvent("断开Socket连接")
    }
